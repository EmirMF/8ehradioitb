...
import { rotateAzuraCastSourcePassword } from "@/lib/azuracast";
import {
...
function shouldRotateSourcePassword() {
  return cleanEnv(process.env.BROADCAST_SERVER_ROTATE_SOURCE_PASSWORD)
    .toLowerCase() !== "false";
}
...
function getBroadcastServerResponseConfig() {
  return {
    ...
    rotatesSourcePassword: shouldRotateSourcePassword(),
  };
}
...
async function requestAzuraCastHealth(url, apiKey) {
  ...
  try {
    ...
    if (!res.ok) {
      return {
        ok: false,
        message: `AzuraCast health returned ${res.status}`,
      };
    }
    ...
    return {
      ...
      message:
        frontend === false
          ? "AzuraCast frontend is not running yet"
    ...
    return {
      ...
      message:
        error.name === "AbortError"
          ? "AzuraCast health timed out"
          : error.message || "AzuraCast health failed",
    };
  } finally {
...
async function checkAzuraCastHealth(activeServerIp) {
  ...
  if (!baseUrl || !stationId || !apiKey) {
    return {
      ok: false,
      message: "AzuraCast health env is missing",
    };
  }
  ...
  if (publicResult.ok) {
    return {
      ok: true,
      message: "AzuraCast public URL is reachable",
    };
  }
...
async function advanceCreating(state) {
  ...
  try {
    ...
    if (error.status === 404) {
      ...
      if (createdRecently) {
        return updateState({
          phase: "hetzner:waiting-for-server",
          lastError: "Hetzner server is not visible yet; waiting.",
        });
      }
...
async function advanceBooting() {
  ...
  if (shouldRotateSourcePassword() && !state.sourcePasswordRotatedAt) {
  ...
  return updateState({
    ...
    phase: "ready",
    lastError: null,
    sourcePasswordRotatedAt: shouldRotateSourcePassword()
      ? new Date()
      : state.sourcePasswordRotatedAt,
  });
}
...
export async function endBroadcastServer(userEmail, options = {}) {
  ...
  if (!snapshot.snapshotId) {
    throw createError("Hetzner did not return a snapshot ID", 502);
  }
...
export async function runBroadcastServerWatchdog() {
  ...
  if (state.status === "running") {
    const autoEndAt = state.autoEndAt
      ? new Date(state.autoEndAt)
      : state.startedAt
        ? new Date(
    ...
    if (autoEndAt && Date.now() >= autoEndAt.getTime()) {
      await endBroadcastServer("watchdog", { bypassMinRuntime: true });
      actions.push("auto-end");
