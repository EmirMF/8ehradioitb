...
import { useLiveChat } from "@/app/hooks/useLiveChat"; // sesuaikan path hook kamu
...
const GlobalAudioPlayer = () => {
  ...
  // Cek apakah guest sudah punya nama (session)
  const [guestName, setGuestName] = useState(null);

  useEffect(() => {
    const savedName = localStorage.getItem("guestName"); // sesuaikan key session kamu
    if (savedName) setGuestName(savedName);
  }, []);

  // Hook real-time chat (sesuaikan dengan implementasi useLiveChat kamu)
    ...
    <>
      {isVisible && (
        <div className="fixed bottom-0 left-0 right-0 z-50">
          ...
          {showLiveChat && (
            <div className="absolute bottom-full right-2 md:right-60 mb-2 w-[340px] md:w-[380px] h-[440px] bg-white border border-gray-200 rounded-lg shadow-2xl flex flex-col overflow-hidden">
              ...
              <div className="flex-1 flex flex-col overflow-hidden relative">
                {!guestName ? (
                  // Belum isi nama → tampilkan modal di dalam popup
                  <GuestNameModal onSaveName={handleNameSubmit} />
                ) : (
                  <>
                    <LiveChatWindow messages={messages} />
                    <ChatInputBox
                      onSend={(text) => sendMessage(text, guestName)}
                    />
                  </>
                )}
              </div>
          ...
          {/* Player UI layer — TIDAK ADA PERUBAHAN dari sebelumnya */}
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${
                    showLiveChat
                      ...
                      : "ring-1 ring-gray-300 hover:ring-gray-900 text-gray-700"
                  }`}
